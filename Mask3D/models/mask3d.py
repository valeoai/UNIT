...
class Mask3D(nn.Module):
    def __init__(
        ...
        self.classification = num_classes > 0
        self.mlp = mlp
    ...
    def forward(
        self, x, queries=None, raw_coordinates=None, is_eval=False, queries_dropout=0.
    ):
        ...
        if queries is None:
            ...
            # parametric queries only for now
            queries, query_pos = queries

        predictions_class = []
        predictions_mask = []

        if queries_dropout > 0:
            # Calculate the number of elements to select per batch
            k = int(queries_dropout * queries.shape[1])

            # Create a tensor of indices and shuffle them for each batch
            indices = torch.rand(queries.shape[0], queries.shape[1]).argsort(dim=1)

            # Select the first k indices from the shuffled indices
            selected_indices = indices[:, k:].to(queries.device).unsqueeze(-1).expand(-1, -1, queries.shape[2])

            # Gather the selected elements from the original tensor
            queries = torch.gather(queries, 1, selected_indices)
            query_pos = torch.gather(query_pos, 0, selected_indices.transpose(0, 1))
        ...
        if self.mlp:
            ...
            # Careful, only available for param queries
            queries = queries + self.query_feat.weight.unsqueeze(0).repeat(
                    batch_size, 1, 1
                )
