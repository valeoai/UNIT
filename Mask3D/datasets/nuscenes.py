# nuScenes dataloader on all frames (sweeps + samples)
...
def index_search(samples_list, sweeps_list, pattern, previous_index=0):
    if pattern.startswith('samples'):
        if f"{pattern[18:-4]}_{previous_index}.seg" in samples_list:
            return previous_index
        for ind, i in enumerate(samples_list):
            if pattern[18:-4] in i:
                samples_list.pop(ind)
                return int(i.rsplit('_', 1)[1].split('.')[0])
    elif pattern.startswith('sweeps'):
        if f"{pattern[17:-4]}_{previous_index}.seg" in sweeps_list:
            return previous_index
        for ind, i in enumerate(sweeps_list):
            if pattern[17:-4] in i:
                sweeps_list.pop(ind)
                return int(i.rsplit('_', 1)[1].split('.')[0])
    for i in range(10):
        print(pattern)
...
class LidarDataset(Dataset):
    def __init__(
        ...
        samples_list = sorted(os.listdir(Path(data_dir) / "assets" / segments_dir / 'samples' / 'LIDAR_TOP'))
        sweeps_list = sorted(os.listdir(Path(data_dir) / "assets" / segments_dir / 'sweeps' / 'LIDAR_TOP'))
        print("Finished loading samples and sweeps")
        index = 0
        for scene in nusc.scene:
            ...
            while current_sample_token != "":
                ...
                index = index_search(samples_list, sweeps_list, current_sample_data['filename'])
